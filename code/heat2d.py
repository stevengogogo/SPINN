...
yspan = [0, 5]
a = 0.1
sigma = lambda x,t: 2*torch.sin(torch.pi*x)
class Heat2D(RegularPDE):
    ...
    def pde(self, x, y, u, ux, uy, uxx, uyy):
        return uy - a * uxx - sigma(x,y)

    def has_exact(self):
        return True

    def exact(self, x, y):
        return np.exp(-4*np.pi**2*a*y)*np.sin(2*np.pi*x) + 2/(np.pi**2 * a) * (1 - np.exp(-np.pi**2*a*y))*np.sin(np.pi*x)
